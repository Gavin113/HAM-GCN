...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        if self.split == 'train':
            # self.data = npz_data['x_train']/[512.0,512.0,1.0]
            self.data = npz_data['x_train']
            self.label = npz_data['y_train']
            self.sample_name = ['train_' + str(i) for i in range(len(self.data))]
            # mask_data_path = 'data/NMFs-CSL/nmf_train_mask.npz'
            self.mask = np.load(self.mask_path)['mask']
        elif self.split == 'test':
            # self.data = npz_data['x_test']/[540.0,540.0,1.0]
            self.data = npz_data['x_test']
            self.label = npz_data['y_test']
            self.sample_name = ['test_' + str(i) for i in range(len(self.data))]
            # mask_data_path = 'data/NMFs-CSL/nmf_test_mask.npz'
            self.mask = np.load(self.mask_path)['mask']
        else:
        ...
        print(self.split,'mask:',self.mask.shape)
